...
const { getOwasp10, extractRisks, extractCwes } = require('../src/owasp10')
const fs = require('fs').promises // Import the promises version of fs

describe('owasp10', () => {
  it('should correctly combine CWE IDs', () => {
    // Exercise
    const extractRisksCweIdsResult = getOwasp10(
    ...
    fs.writeFile(
      './owasp10.json',
      JSON.stringify(extractRisksCweIdsResult, null, 2)
    )

    // Verify
    expect(extractRisksCweIdsResult).toMatchSnapshot()
  })
})

test('extracts data from ../test/sample_data/sample_index.md', () => {
  const extractControlsResult = extractRisks(
    '../test/sample_data/sample_index.md'
  )
...
test('extracts data from ../test/sample_data/sample_risk.md', () => {
  const extractCweIdsResult = extractCwes('../test/sample_data/sample_risk.md')
